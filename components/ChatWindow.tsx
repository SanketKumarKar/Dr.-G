...
const ChatMessage: React.FC<{ message: Message }> = ({ message }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`max-w-md p-4 rounded-xl shadow-sm relative overflow-hidden ${isModel ? 'bg-indigo-50 text-slate-800' : 'bg-white border border-slate-200/80 text-slate-800'}`}>
...
const ThinkingIndicator: React.FC = () => (
    <div className="flex items-start gap-3 my-5">
        ...
        <div className="max-w-md p-4 rounded-xl shadow-sm bg-indigo-50 text-slate-800 animate-pulse">
            <div className="flex items-center gap-2">
                <Spinner className="w-4 h-4 text-indigo-600" />
                <span className="text-sm font-medium text-slate-600">Dr.G is thinking...</span>
            </div>
        </div>
    </div>
);
...
const ChatWindow: React.FC<ChatWindowProps> = ({ messages, onSendMessage, isLoading, onReviewSymptoms, isReviewing, isChatting, apsa }) => {
  ...
  const buildContextualAnswer = (raw: string): string => {
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-slate-50 to-white rounded-2xl shadow-xl border border-slate-200/70 flex flex-col overflow-hidden ring-1 ring-black/5 backdrop-blur">
        <div className="p-4 border-b border-slate-200/60 flex-shrink-0 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/40">
             <DrGLogo />
        </div>
            ...
            {symptomOptions.length > 1 && (
              <div className="mt-3">
                <OptionChips
                  ...
                  disabled={isLoading}
                  onSelect={(val) => {
                    if (val === 'None of these') handleQuickSelect('No other symptoms');
                    else handleQuickSelect(val);
                  }}
                />
              </div>
            )}
  ...
  {isChatting && messages.length > 1 && (
         <div className="px-6 pb-4 text-center">
          <button
            ...
            className="px-6 py-2.5 bg-white text-indigo-700 rounded-lg font-semibold text-sm hover:bg-indigo-50 transition-all border border-indigo-200 shadow-sm flex items-center justify-center mx-auto gap-2 disabled:opacity-50"
          >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
  ...
  <div className="mt-auto p-4 border-t border-slate-200/60 bg-white/80 backdrop-blur">
            <div className="relative">
                <textarea
                    ...
                    rows={1}
                    className="w-full p-3 pr-12 bg-slate-100/80 rounded-xl text-sm text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none overflow-y-hidden transition-all shadow-inner"
                    disabled={isLoading || isReviewing}
                />
                <button
                    onClick={handleSend}
                    disabled={!input.trim() || isLoading || isReviewing}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-gradient-to-br from-indigo-600 to-violet-600 text-white rounded-xl flex items-center justify-center hover:from-indigo-500 hover:to-violet-500 disabled:bg-slate-400 disabled:cursor-not-allowed transition-all shadow focus:outline-none focus:ring-4 focus:ring-indigo-300"
